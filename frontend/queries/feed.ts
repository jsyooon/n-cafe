...
const fetchFeedList = (cookie?: CookieItemType) =>
  fetcher<FeedPreviewList>('/feeds', 'GET', cookie ? { cookie } : {}).then((response) => response?.data);
...
export const fetchFeedItem = (id: FeedItem['id'], cookie?: CookieItemType) =>
  fetcher<FeedItem>(`/feed/${id}`, 'GET', cookie ? { cookie } : {}).then((response) => response?.data);
